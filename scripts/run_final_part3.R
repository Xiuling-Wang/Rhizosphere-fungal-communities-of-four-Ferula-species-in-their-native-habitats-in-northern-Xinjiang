## Part 3: FungalTraits functional annotation + Core genera + PERMDISP (corrected set).
...
ad <- file.path(P,"asv_trial"); out <- file.path(P,"downstream_fungi_final"); tb<-file.path(out,"tables"); fg<-file.path(out,"figures")
rr<-read.delim(file.path(tb,"asv_table_fungi_rarefied_3819.tsv"),check.names=FALSE); rare<-as.matrix(rr[,-1]); rownames(rare)<-rr$sample
tax<-read.delim(file.path(ad,"taxonomy/asv_taxonomy_fungiBLASTset_UNITE.tsv"),check.names=FALSE); rownames(tax)<-tax$asv_id
meta<-read.delim(file.path(tb,"sample_metadata_downstream.tsv"),check.names=FALSE); meta<-meta[match(rownames(rare),meta$sample),]
meta$site<-factor(meta$site,levels=c("HD","XJ","DS","DG"))
FT<-read.csv(file.path(ad,"taxonomy/fungaltraits/polme2020_genera.csv"),check.names=FALSE)
...
## (2) biomarker guilds
bm<-read.delim(file.path(tb,"genus_biomarkers_final.tsv"),check.names=FALSE)
