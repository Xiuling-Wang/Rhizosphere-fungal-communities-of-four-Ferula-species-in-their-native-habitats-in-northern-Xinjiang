## Polished figures for revised Rhizosphere submission.
## This script does not change statistics; it only improves figure aesthetics.
...
## NOTE: final analysis tables are expected in <root>/tables (copy them from
## analysis/downstream_fungi_final/tables after running run_final*.R).
ad <- file.path(root, "analysis", "asv_trial")
tb <- file.path(root, "tables")
fg <- file.path(root, "figures")
dir.create(fg, recursive = TRUE, showWarnings = FALSE)

rr <- read.delim(file.path(tb, "asv_table_fungi_rarefied_3819.tsv"), check.names = FALSE)
rare <- as.matrix(rr[, -1]); rownames(rare) <- rr$sample
tax <- read.delim(file.path(ad, "taxonomy/asv_taxonomy_fungiBLASTset_UNITE.tsv"), check.names = FALSE)
rownames(tax) <- tax$asv_id
meta <- read.delim(file.path(tb, "sample_metadata_downstream.tsv"), check.names = FALSE)
...
pres  <- rare > 0
sets  <- lapply(levels(meta$site), function(s)
  colnames(pres)[colSums(pres[meta$site == s, , drop = FALSE]) > 0])
names(sets) <- levels(meta$site)
venn_file <- file.path(fg, "Figure2D_venn_polished.png")
futile.logger::flog.threshold(futile.logger::ERROR, name = "VennDiagramLogger")
## Clean, airy four-set Venn (Lucid-style): light pastel fills so the
## four-way overlap stays light instead of muddy, crisp thin coloured
## outlines for emphasis, and quiet dark-slate count labels.
venn_fill <- c(HD = "#BFE3EE", XJ = "#F6BDB4", DS = "#A6DDD2", DG = "#D8C2E2")
venn_edge <- site_cols  # crisp saturated edges keep each set identifiable
venn.diagram(
  sets[c("HD", "DG", "XJ", "DS")],
  filename   = venn_file, imagetype = "png",
  height = 2600, width = 2600, resolution = 600,
  fill   = venn_fill[c("HD", "DG", "XJ", "DS")],
  alpha  = 0.62,
  col    = venn_edge[c("HD", "DG", "XJ", "DS")],
  lwd = 1.6, cex = 1.05, fontface = "plain", fontfamily = "Arial",
  cat.cex = 1.35, cat.fontface = "bold", cat.fontfamily = "Arial",
  cat.col = venn_edge[c("HD", "DG", "XJ", "DS")],
  margin = 0.11, main = NULL
)
venn_img <- png::readPNG(venn_file)
pD <- ggplot() +
  annotation_custom(rasterGrob(venn_img, interpolate = TRUE),
...
fig2 <- plot_grid(
  ...
  pD,
  labels = c("A", "B", "C", "D"),
  label_fontface = "bold", label_size = 13,
  ncol = 2, align = "hv"
)
...
depth_method_caption <- paste0(
  "Within-site depth comparisons: Kruskal-Wallis per species, with pairwise Wilcoxon (Benjamini-Hochberg) post hoc where significant (p < 0.05).\n",
  "Letters mark significant depth contrasts; none were detected, so no letters are shown. Comparisons were exploratory and not adjusted across species."
)
...
p_alpha_one <- function(metric_label, caption = NULL) {
  ...
  ggplot(d, aes(depth_order, value, fill = depth_order, colour = depth_order)) +
    geom_boxplot(width = 0.55, outlier.shape = NA, linewidth = 0.4, fatten = 1.4, alpha = 0.65) +
    geom_jitter(width = 0.14, height = 0, size = 1.6, stroke = 0.28, shape = 21, alpha = 0.85) +
    ...
    theme(
      ...
      plot.caption      = element_text(size = 5.6, hjust = 0, colour = "grey25",
                                       lineheight = 1.05, margin = margin(t = 3))
    )
}
...
draw_fig3c <- function() {
  corrplot(cc, method = "color", type = "lower", order = "original",
           ...
           pch.col = "#222222", pch.cex = 1.18, mar = c(0, 0, 1, 0))
}
pdf(file.path(fg, "Fig3C_alpha_soil_corr_polished.pdf"), width = 7.4, height = 7.1, bg = "white")
draw_fig3c(); dev.off()
png(file.path(fg, "Fig3C_alpha_soil_corr_polished.png"), width = 7.4, height = 7.1,
    units = "in", res = 450, bg = "white")
draw_fig3c(); dev.off()

pC <- ggplot() +
  annotation_custom(
    rasterGrob(png::readPNG(file.path(fg, "Fig3C_alpha_soil_corr_polished.png")),
               interpolate = TRUE),
    xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf) +
  theme_void() +
  theme(plot.background = element_rect(fill = "white", colour = NA))
...
fig3_combined <- plot_grid(
  plot_grid(
    ...
    labels = c("A", "B"), ncol = 1,
    label_fontface = "bold", label_size = 12, align = "v"
  ),
  pC,
  labels = c("", "C"), ncol = 2,
...
ht4a <- Heatmap(
  ...
  width  = unit(6.5, "cm"),
  height = unit(n_cls * 0.58, "cm")   # dynamic: ~0.58 cm per row
)
...
fig4a_w <- 9.0                          # inches: wider heatmap body (6.5 cm) + annotations + legends
fig4a_h <- (n_cls * 0.58 / 2.54) + 4.2   # inches: heatmap body + margins + column dendrogram
...
## Panel labels A / B (B nudged down to clear the "Phylum" strip title)
grid.text("A", x = unit(0.012, "npc"), y = unit(0.99, "npc"),
          just = c("left", "top"), gp = gpar(fontsize = 12, fontface = "bold"))
grid.text("B", x = unit(panelA_frac + 0.012, "npc"),
          y = unit((fig4_combined_h_in / 2 + fig4b_h_in / 2) / fig4_combined_h_in, "npc") - unit(1, "mm"),
          just = c("left", "top"), gp = gpar(fontsize = 12, fontface = "bold"))
...
label_map <- c(Nitrate_N = "Nitrate-N", Ammonium_N = "Ammonium-N",
...
perm_label <- sprintf(
  "PERMANOVA (adonis2)\nSpecies/site (plant-level): R² = %.3f, p = %.3f%s\nDepth (within-plant): R² = %.3f, p = %.3f%s",
...
hp_label_map <- c(Nitrate_N = "Nitrate-N", Ammonium_N = "Ammonium-N",
...
env_labels <- c("SM","OM","TN","TP","TK","NO3-N","NH4-N","Olsen-P","AK","pH","EC","TDS")
...
colnames(rho) <- env_labels; colnames(pv5c) <- env_labels
...
cell_fn <- function(j, i, x, y, width, height, fill) {
  ...
  if (sym != "") {
    grid.text(sym, x, y,
              gp = gpar(fontsize = if (sym == "***") 9.5 else if (sym == "**") 10 else 11,
...
gap_ab  <- 0.20   # vertical gap between A and B
...
figS1 <- plot_grid(pSA, pSB,
  labels = c("A", "B"), label_fontface = "bold", label_size = 13,
...
pS2 <- ggplot(cg, aes(n_core_ASVs, genus_f, fill = primary_lifestyle)) +
  ...
  theme(
    ...
    plot.title      = element_markdown(size = 8.5, face = "plain",
                                       margin = margin(b = 6))
  )
...
## ─────────────────────────────────────────────
## Finalize: assemble the numbered submission set in figures/
##   01_Fig1.pdf and 07_FigS2.pdf are static (sourced from figure_V1/, not
##   regenerated by this script); everything else below comes from this run.
##   Individual panel/component PDFs (and intermediate PNGs) are moved into
##   figures/components/ so figures/ only ever holds the current 01-10
##   submission set — re-running this script will not leave stale
##   duplicates behind.
## ─────────────────────────────────────────────
comp_dir <- file.path(fg, "components")
dir.create(comp_dir, recursive = TRUE, showWarnings = FALSE)

## Static reference figures, kept in sync from figure_V1/
static_map <- c(
  "Fig1.pdf"  = "01_Fig1.pdf",
  "FigS2.pdf" = "07_FigS2.pdf"
)
for (src in names(static_map)) {
  sp <- file.path(root, "figure_V1", src)
  if (file.exists(sp)) file.copy(sp, file.path(fg, static_map[src]), overwrite = TRUE)
}
...
for (src in names(final_map)) {
  file.copy(file.path(fg, src), file.path(fg, final_map[src]), overwrite = TRUE)
}

## Individual panel/component PDFs and intermediate PNGs -> figures/components/
component_files <- c(
  "Fig3AB_alpha_polished.pdf", "Fig3C_alpha_soil_corr_polished.pdf",
  "Fig3C_alpha_soil_corr_polished.png", "Figure2D_venn_polished.png",
  "Fig4A_class_heatmap_polished.pdf", "Fig4B_biomarkers_polished.pdf",
  "Fig5A_dbRDA_polished.pdf", "Fig5B_varpart_polished.pdf", "Fig5C_genus_soil_polished.pdf",
  names(final_map)
)
for (src in component_files) {
  sp <- file.path(fg, src)
  if (file.exists(sp)) file.rename(sp, file.path(comp_dir, src))
}

cat("\nSubmission set (01_Fig1 ... 10_FigS5) written to:\n", fg, "\n")
cat("Component/intermediate files moved to:\n", comp_dir, "\n")
